...
--
-- Stand-in structure for view `ratingcommentview1`
-- (See below for the actual view)
--
CREATE TABLE `ratingcommentview1` (
`id_rating` int(20)
,`userid` int(20)
,`id_rm` int(10)
,`Rating` int(1)
...
--
-- Structure for view `ratingcommentview1`
--
DROP TABLE IF EXISTS `ratingcommentview1`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ratingcommentview1`  AS SELECT `rating`.`id_rating` AS `id_rating`, `rating`.`userid` AS `userid`, `rating`.`id_rm` AS `id_rm`, `rating`.`Rating` AS `Rating`, `user`.`username` AS `username`, `komentar`.`id_komentar` AS `id_komentar`, `komentar`.`Komentar` AS `Komentar` FROM ((`rating` join `komentar`) join `user`) WHERE `rating`.`id_rm` = `komentar`.`id_rm` AND `user`.`userid` = `rating`.`userid` AND `user`.`userid` = `komentar`.`userid``userid`  ;
...
--
ALTER TABLE `rating`
  MODIFY `id_rating` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
